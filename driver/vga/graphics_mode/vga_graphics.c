...
void init_vga() {
	...
	bool result = write_registers(g_320x200x256);
	if (result == true) {
		/*terminal_setcolor(VGA_GREEN);
		printf("[+]: Passe au mode graphique avec succes.\n");
		terminal_color(VGA_WHITE);*/
		serial_print("[+]: Mode graphique chargé avec succès.\n", SERIAL_COM1_PORT);
	}

	draw_rect(320, 200, 0, 0, 0);
}
...
void draw_char(uint8_t character, uint16_t pos_x, uint16_t pos_y, uint8_t color) {
	...
	uint8_t *glyph = g_8x16_font + (int)character * 16;
